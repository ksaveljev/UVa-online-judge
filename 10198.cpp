...
int main(void)
{
  int n;
  bignum tmp1, tmp2;
  bignum *result = new bignum[1001];

  int_to_bignum(1, &result[0]);
  int_to_bignum(2, &result[1]);
  int_to_bignum(5, &result[2]);

  for (int i = 3; i < 1001; i++) {
    add_bignum(&result[i-1], &result[i-1], &tmp1);
    add_bignum(&result[i-2], &tmp1, &tmp2);
    add_bignum(&result[i-3], &tmp2, &result[i]);
  }
  
  while (cin >> n) {
    print_bignum(&result[n]);
  }
