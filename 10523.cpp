...
int main(void)
{
  ...
  tmp = new bignum*[16];
  for (int i = 0; i < 16; i++) {
    tmp[i] = new bignum[151];
  }

  for (int i = 0; i < 16; i++) {
    int_to_bignum(1, &tmp[i][0]);
  }


  bignum tmp_j, tmp_result, tmp_a, zero, tmp_i;
  int_to_bignum(0, &zero);
  for (int i = 1; i < 16; i++) {
    int_to_bignum(i, &tmp_i);
    for (int j = 1; j < 151; j++) {
      multiply_bignum(&tmp[i][j-1], &tmp_i, &tmp_result);
      add_bignum(&zero, &tmp_result, &tmp[i][j]);
    }
  }

  for (int i = 1; i < 16; i++) {
    for (int j = 1; j < 151; j++) {
      int_to_bignum(j, &tmp_j);
      multiply_bignum(&tmp_j, &tmp[i][j], &tmp_result);
      add_bignum(&zero, &tmp_result, &tmp[i][j]);
    }
  }

  for (int i = 0; i < 16; i++) {
    int_to_bignum(0, &tmp[i][0]);
  }

  for (int i = 1; i < 16; i++) {
    for (int j = 1; j < 151; j++) {
      add_bignum(&tmp[i][j-1], &tmp[i][j], &tmp_result);
      add_bignum(&tmp_result, &zero, &tmp[i][j]);
    }
  }
